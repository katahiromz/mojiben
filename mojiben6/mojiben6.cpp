...
BOOL OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
    ...
    {
        g_ahbmKanji3[j] = LoadGif(g_hInstance, 1000 + j);
...
VOID OnDraw(HWND hwnd, HDC hdc)
{
    ...
    if (g_hbmClient == NULL)
    {
        ...
        {
            GetMojiRect(j, &rc);
            hbmOld = SelectObject(hdcMem, g_ahbmKanji3[j]);
...
static unsigned ThreadProcWorker(void)
{
    ...
    const std::vector<STROKE>& v = g_kanji3_kakijun[g_nMoji];
...
void OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    g_kanji3_history.insert(g_nMoji);
    if (g_hbmClient)
    {
        DeleteObject(g_hbmClient);
        g_hbmClient = NULL;
