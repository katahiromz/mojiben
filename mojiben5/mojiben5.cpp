...
void EnumData() {
    ...
    for (size_t i = 0; i < g_pMoji->size(); ++i) {
...
#if 0
        std::vector<STROKE> v = g_kanji2_kakijun[i];
...
#if 0
        {
            ...
            INT ires = 0;
            for (size_t k = 0; k < g_kanji2_kakijun[i].size(); ++k) {
                if (g_kanji2_kakijun[i][k].type != STROKE::WAIT) {
                    DWORD size;
...
INT GetNumPage(VOID)
{
    return (g_ahbmMoji.size() + COLUMNS * ROWS - 1) / (COLUMNS * ROWS);
}
...
void OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    g_kanji2_history.insert(g_nMoji);
    if (g_hbmClient)
    {
        DeleteObject(g_hbmClient);
        g_hbmClient = NULL;
