...
void EnumData() {
    ...
    for (size_t i = 0; i < g_pMoji->size(); ++i) {
...
#if 0
        std::vector<STROKE> v = g_kanji6_kakijun[i];
...
#if 0
        {
            ...
            INT ires = 0;
            for (size_t k = 0; k < g_kanji6_kakijun[i].size(); ++k) {
                if (g_kanji6_kakijun[i][k].type != STROKE::WAIT) {
                    DWORD size;
...
HRGN MyCreateRegion(INT nIndex, INT iKakijun, INT i, INT ires) {
    ...
    return DeserializeRegion254((const BYTE *)pvData, cbData);
#endif
}
