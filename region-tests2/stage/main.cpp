...
int main(void)
{
    ...
    HANDLE hFind = FindFirstFileA("RCData_*.bin.bmp", &find);
    ...
    do
    {
        ...
        cv::Mat src = cv::imread(find.cFileName, cv::IMREAD_GRAYSCALE);
        if (src.empty()) {
            printf("Cant load\n");
            return -1;
        }
        ...
        cv::Mat dst;
        cv::GaussianBlur(src, src, cv::Size(3, 3), 0.5);
        cv::resize(src, dst, cv::Size(254, 254), 0, 0, cv::INTER_AREA);
